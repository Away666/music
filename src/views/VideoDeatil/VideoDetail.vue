...
export default {
  ...
  methods: {
    ...
    // 根据传来的id查询mv详情
    async getMvDetail() {
      let res = await this.$http("/mv/detail", {
    ...
    // 请求mv地址
    async getMvUrl() {
      let res = await this.$http("/mv/url", { id: this.$route.params.id });
      this.videoUrl = res.data.data.url;
    },
    // 请求相关视频
    async getRelatedVideo() {
      let res = await this.$http("/related/allvideo", {
    ...
    async getMvComment(type) {
      ...
      let res = await this.$http("/comment/mv", {
        id: this.$route.params.id,
    ...
    // 滚动到输入框
    scrollToComment() {
      let videoDetail = document.querySelector(".el-main");
    ...
    async getVideoDetail() {
      ...
      let res = await this.$http("/video/detail", {
        id: this.$route.params.id,
      });
      this.videoInfo = res.data.data;
      console.log("视频详情");
    ...
    // 获取视频播放地址
    async getVideoUrl() {
      let res = await this.$http("/video/url", {
        id: this.$route.params.id,
      });
      //   console.log(res);
    ...
    async getVideoComment(type) {
      ...
      let res = await this.$http("/comment/video", {
        id: this.$route.params.id,
        offset: 20 * (this.commentsPage - 1),
        timestamp,
      });
    ...
    async getSubVideoList() {
      ...
      let res = await this.$http("/mv/sublist", { limit: 1000, timestamp });
      if (res.data.code != 200) {
    ...
    async getLikeVideoList() {
      let timestamp = Date.parse(new Date());
      let res = await this.$http("/playlist/mylike", {
        limit: 1000,
        timestamp,
      });
      // console.log(res);
    ...
    async subVideo() {
      ...
      if (this.$route.params.type == "video") {
        await this.$http("/video/sub", {
          id: this.$route.params.id,
          t: this.isSub ? 1 : 0,
        });
      } else {
        await this.$http("/mv/sub", {
          mvid: this.$route.params.id,
          t: this.isSub ? 1 : 0,
          timestamp,
        });
    ...
    async likeVideo() {
      ...
      // 请求
      let res = await this.$http("/resource/like", {
        type,
        id: this.$route.params.id,
        t: this.isLike ? 1 : 0,
        timestamp,
