<template>
  ...
  >
    ...
    <div
      class="musicDetailCardContainer scroll"
...
export default {
  ...
  methods: {
    ...
    //请求并处理歌词数据
    async getLyric(id) {
      let res = await this.$http("/lyric", { id });
      //   console.log(res);
      ...
      arr.forEach((item) => {
        ...
        console.log("time" + parseFloat(t[1]));
      });
    ...
    async getMusicComment(id, type) {
      ...
      if (this.currentCommentPage == 1) {
        let res = await this.$http("/comment/hot", {
          id,
          limit: 10,
          type: 0,
          timestamp,
      ...
      let res = await this.$http("/comment/music", {
        id,
