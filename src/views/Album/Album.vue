...
export default {
  ...
  methods: {
    ...
    async getAlbumDetail() {
      let res = await this.$http("album", { id: this.$route.params.id });
      res = res.data;
      // 处理歌曲时间
    ...
    async getAlbumComment(type) {
      ...
      let res = await this.$http("/comment/album", {
    ...
    scrollToComment() {
      ...
      let info = document.querySelector(".listInfo");
    ...
    async getAlbumSublist() {
      ...
      let res = await this.$http("/album/sublist", {
        limit: 1000,
        timestamp,
      });
      console.log(res);
    ...
    async clickCell(row, column, cell) {
      ...
      if (cell.querySelector(".icon-download")) {
        // 请求该歌曲的url
        console.log(row);
        let res = await this.$http("/song/url", { id: row.id });
        console.log(res);
