        ...
        <el-tab-pane label="专辑" name="first">
          <div class="topSongs" v-if="topSongs.topSongs">
            ...
            <list-table
              ...
              :cover="require('/src/assets/images/topSongsLogo.png')"
...
export default {
  ...
  methods: {
    ...
    // 请求歌手详情
    async getSingerInfo() {
      let res = await this.$http("/artist/detail", {
    ...
    async getSingerTopSongs() {
      let res = await this.$http("/artist/top/song", {
        id: this.$route.params.id,
      });
      // console.log(res);
    ...
    // 请求专辑列表 再用专辑id查询专辑信息
    async getAlbumInfo() {
      let res = await this.$http("/artist/album", {
        id: this.$route.params.id,
        limit: 20,
    ...
    async getAlbumDetail(id) {
      let res = await this.$http("/album", { id });
    ...
    async getSingerMv() {
      // 这个接口没有offset 参数
      let res = await this.$http("/artist/mv", {
        id: this.$route.params.id,
    ...
    async getSubSingerList() {
      ...
      let res = await this.$http("/artist/sublist", {
        limit: 1000,
        timestamp,
      });
